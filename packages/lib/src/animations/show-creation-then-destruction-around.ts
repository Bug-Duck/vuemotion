...
export const showCreationThenDestructionAround = defineAnimation<{
  ...
  if (progress >= 0 && progress < 0.5)
    context.rect?.setAttribute("stroke-dasharray",
      `${interpolator(0, context.target.range?.width! * 2 + context.target.range?.height! * 2, progress * 2)},${interpolator(context.target.range?.width! * 2 + context.target.range?.height! * 2, 0, progress * 2)}`)
  else if (progress >= 0.5 && progress < 1)
    context.rect?.setAttribute("stroke-dasharray",
      `${interpolator(0, context.target.range?.width! * 2 + context.target.range?.height! * 2, 1 - (progress - 0.5) * 2)},${interpolator(context.target.range?.width! * 2 + context.target.range?.height! * 2, 0, 1 - (progress - 0.5) * 2)}`)
  if (progress === 1)
    context.rect?.remove()
})